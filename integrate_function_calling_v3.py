#!/usr/bin/env python3
"""
Function Calling集成脚本 V3
完整的三层回退：DeepSeek → Doubao → Zhipu
"""
...
# 新的generateAiResponse - 完整三层回退
NEW_GENERATE_AI_RESPONSE = '''    /**
     * 使用Function Calling生成AI回复
     * 三层回退：DeepSeek (Function Calling) → Doubao → Zhipu
     */
    private String generateAiResponse(String userMessage, List<Map<String, String>> history) {
        ...
        // 4. DeepSeek (Function Calling)
        if (deepSeekService.isAvailable()) {
            try {
                log.info("【AI聊天服务】尝试DeepSeek + Function Calling");
                ...
                if (!response.containsKey("error")) {
                    if (response.containsKey("tool_calls")) {
                        String result = handleFunctionCalls(response, messages, functions);
        ...
        try {
            ...
            log.warn("【AI聊天服务】Doubao失败，切换到Zhipu");
        } catch (Exception e) {
            log.warn("【AI聊天服务】Doubao异常: {}, 切换到Zhipu", e.getMessage());
        }
        
        // 6. Zhipu (最后的回退)
        if (zhipuChatService.isAvailable()) {
            try {
                log.info("【AI聊天服务】尝试Zhipu模型");
                String aiReply = zhipuChatService.chat(userMessage, SYSTEM_PROMPT);
                if (aiReply != null && !aiReply.trim().isEmpty() && !aiReply.startsWith("⚠️")) {
                    return aiReply;
                }
            } catch (Exception e) {
                log.warn("【AI聊天服务】Zhipu也失败了: {}", e.getMessage());
            }
        }
        
        // 7. 所有AI都失败，返回fallback
...
def main():
    ...
    start_marker = "// 系统提示词 - Function Calling版本"
    
    # 找到当前的SYSTEM_PROMPT
    start_idx = content.find(start_marker)
    if start_idx == -1:
        print("  ✗ 未找到SYSTEM_PROMPT标记")
        return
    
    # 找到SYSTEM_PROMPT的结束（下一个方法或字段）
    end_marker = "@Override"
    end_idx = content.find(end_marker, start_idx)
    
    if end_idx == -1:
        print("  ✗ 未找到SYSTEM_PROMPT结束位置")
        return
    
    # 替换
    content = content[:start_idx] + NEW_SYSTEM_PROMPT + "\n\n    " + content[end_idx:]
    ...
    print("  ✓ 完整三层回退：DeepSeek → Doubao → Zhipu")
    ...
    print("\n✅ 所有修改完成！")
    print("   - 简化SYSTEM_PROMPT")
    print("   - Function Calling (DeepSeek)")
    print("   - 完整三层回退机制")
    print("   - 权限逻辑由Service处理")
    print("\n请重启服务测试。")
