...
public class AiCustomerExtractController {
    ...
    @Autowired
    private com.aicustomer.service.ZhipuChatService zhipuChatService;
    ...
    private Map<String, Object> extractWithAI(String text) {
        ...
        try {
            ...
            // 优先使用智谱AI GLM-4-Flash（免费且性能好）
            if (zhipuChatService != null && zhipuChatService.isAvailable()) {
                try {
                    log.info("【客户信息提取】使用智谱AI GLM-4-Flash");
                    String aiResult = zhipuChatService.chat(prompt, systemPrompt);

                    if (aiResult != null && !aiResult.trim().isEmpty()) {
                        log.info("【客户信息提取】智谱AI调用成功，返回结果长度: {}", aiResult.length());

                        // 清理AI返回的结果，提取JSON部分
                        String cleanedResult = cleanJsonResponse(aiResult);
                        log.info("【客户信息提取】清理后的结果: {}",
                                cleanedResult.substring(0, Math.min(200, cleanedResult.length())));

                        // 尝试解析AI返回的JSON结果
                        try {
                            @SuppressWarnings("unchecked")
                            Map<String, Object> parsedJson = (Map<String, Object>) gson.fromJson(cleanedResult,
                                    Map.class);
                            info.put("aiProcessed", true);
                            info.put("aiResult", cleanedResult);
                            info.put("parsedData", parsedJson);
                            info.put("aiProvider", "zhipu-glm-4-flash");
                            log.info("【客户信息提取】JSON解析成功（智谱AI）");
                            return info;
                        } catch (Exception e) {
                            log.warn("【客户信息提取】智谱AI返回的结果不是有效的JSON格式: {}", cleanedResult);
                    ...
                    log.warn("【客户信息提取】智谱AI调用失败，尝试使用豆包: {}", e.getMessage());
                }
            }

            // 使用Spring AI框架调用豆包模型（备用方案）
            try {
                log.info("【客户信息提取】使用豆包AI（备用方案）");
